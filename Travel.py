...
while(flag==0):
	while(True):
		...
		if choice==1:
			...
			response = requests.get("https://maps.googleapis.com/maps/api/geocode/json?address="+inp+"&key=Your google maps api")
	...
	response = requests.get("https://maps.googleapis.com/maps/api/geocode/json?address="+destination+"&key=Your google maps api")
	resp_json_payload = response.json()
	dest_address=[]
	c=resp_json_payload['results'][0]['geometry']['location']['lat']
